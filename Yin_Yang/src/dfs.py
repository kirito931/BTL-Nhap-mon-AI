...
MỤC ĐÍCH: Hiện thực thuật toán Depth-First Search (DFS) kết hợp Backtracking,
          Suy diễn tất định (Constraint Propagation) và Cắt tỉa không gian trạng thái (Pruning)
          để giải bài toán Yin-Yang (Âm Dương).
NHIỆM VỤ:
- Suy diễn ràng buộc khối 2x2 (chống 4 ô cùng màu & chống bẫy ca-rô).
- Áp dụng Định lý Chu vi (Perimeter Theorem): số lần đổi màu dọc biên <= 2 và lan truyền cung viền.
- Áp dụng Định lý Cầu nối liên thông (Bridge / Exit Rule): phát hiện đảo chết và ép ô thoát duy nhất.
- Suy diễn quy tắc bao vây (Surrounded Cells) & mẫu góc 2x3, 3x2.
- Chọn biến phân nhánh theo Heuristic MRV (Minimum Remaining Values) có bậc ràng buộc cao nhất.
...
class DFSSolver:
    ...
    def __init__(self, board: YinYangBoard, max_nodes: int = None, timeout_sec: float = None, cancel_event=None, step_callback=None):
        ...
        # Ngăn xếp phục vụ hoàn tác (Backtrack stack): (r, c, old_val, action_type)
        self._stack = []
        ...
        # Mảng đánh dấu BFS để tối ưu hóa hiệu năng
    ...
    def _shave_pass(self):
        """Domain lookahead (Singleton Consistency): kiểm tra các ô kề ô đã điền, ép ô có miền giá trị = 1."""
        changed = False
        candidates = []
        for r in range(self.rows):
            for c in range(self.cols):
